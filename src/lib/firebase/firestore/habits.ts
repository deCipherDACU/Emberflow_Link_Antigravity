...
export const createHabitsInBatch = async (db: Firestore, userId: string, habits: Omit<Habit, 'id'>[]): Promise<string[]> => {
    const batch = writeBatch(db);
    const habitsCol = habitsCollection(db, userId);
    const newHabitIds: string[] = [];

    habits.forEach(habitData => {
        const newHabitRef = doc(habitsCol);
        batch.set(newHabitRef, habitData);
        newHabitIds.push(newHabitRef.id);
    });
    ...
    try {
        ...
        const permissionError = new FirestorePermissionError({
            ...
            requestResourceData: { habitCount: habits.length },
        });
        errorEmitter.emit('permission-error', permissionError);
        throw permissionError; // Rethrow to be caught by the service
    }
};
